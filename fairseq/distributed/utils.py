...
from fairseq.utils import is_moe, print_r0
import io
import logging
import os
import pickle
...
def _infer_slurm_init(cfg: DistributedTrainingConfig, num_pipelines_per_node):
    ...
    if node_list is not None:
        host=os.environ.get('SLURM_LAUNCH_NODE_IPADDR', None)
        # try:
        ...
        if ntasks_per_node == 1:
            ...
            if cfg.distributed_gpu_list is None or len(cfg.distributed_gpu_list)==0:
                assert ntasks_per_node == torch.cuda.device_count(), '{} {}'.format(ntasks_per_node, torch.cuda.device_count())
        ...
        if cfg.distributed_gpu_list is not None and len(cfg.distributed_gpu_list)>0:
            distributed_gpu_list=cfg.distributed_gpu_list.split()
            assert ntasks_per_node==len(distributed_gpu_list)
            cfg.device_id=int(distributed_gpu_list[int(os.environ.get("SLURM_LOCALID"))])
        if cfg.distributed_ifname_list is not None and len(cfg.distributed_ifname_list)>0:
            distributed_ifname_list=cfg.distributed_ifname_list.split()
            assert len(distributed_ifname_list)==cfg.distributed_world_size, "only support one gpu on ip now"
            ifname=distributed_ifname_list[cfg.distributed_rank]
            os.environ['NCCL_SOCKET_IFNAME']=ifname
        print('rank:{}, using gpu:{}, using ifname: {}'.format(cfg.distributed_rank, cfg.device_id, os.environ.get('NCCL_SOCKET_IFNAME', None)))
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            if torch.cuda.is_available():
                dist.all_reduce(x.cuda())
                _=x.sum()
                print_r0('dummy test all-reduce success!')
        ...
        if not hasattr(get_moe_group, "_moe_groups"):
            ...
            # larger world than num experts
                ...
                moe_groups = [[i + j * moe_expert_count for j in range(ranks_per_group)]
                                for i in range(moe_expert_count)] # gpu of same expert is grouped togther
        ...
        if not hasattr(get_all2all_group, "_all2all_groups"):
            ...
            # larger world than num experts
                ...
                all2all_groups = [[i * moe_expert_count + j for j in range(moe_expert_count)]
                                    for i in range(ranks_per_group)] # gpu of same all2all group is grouped togther
