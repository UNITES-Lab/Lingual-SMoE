...
class MoECriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logging_output["lang_loss"] = lang_loss.data if torch.is_tensor(lang_loss) else lang_loss
    ...
    def compute_loss(self, model, sample, reduce=True):
        ...
        if self.gate_loss_combine_method == "average":
            ...
            lang_loss = lang_loss / gate_count
        if self.gate_loss_transform == "neg_log":
            gate_loss = - torch.log(gate_loss)

        gate_loss = sample_size * gate_loss
        lang_loss = sample_size * lang_loss

        loss = inner_loss + self.gate_loss_weight * gate_loss + self.gate_loss_weight * lang_loss
    
        return loss, inner_loss, gate_loss, lang_loss, self.get_moe_metadata(model), sample_size, logging_output
    ...
    def reduce_moe_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "lang_loss", lang_loss_sum / sample_size, sample_size, round=8
        )
