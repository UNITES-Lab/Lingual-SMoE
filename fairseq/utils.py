...
def is_moe(cfg):
    return getattr(cfg, "moe_freq", 0) > 0 \
        or getattr(cfg, "encoder_moe_freq", 0) > 0 \
        or getattr(cfg, "decoder_moe_freq", 0) > 0
...
def get_size(obj, seen=None):
    ...
    elif hasattr(obj, '__iter__') and not isinstance(obj, (IOBase, str, bytes, bytearray)):
